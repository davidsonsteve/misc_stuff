...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        # take self.message_text, split it by spaces into a list.
        shiftamt = 0
        matches = 0 #number of valid words
        finalshift = 0
        finaltext = ""
        msglen = 0 
        while shiftamt < 27:
            test = PlaintextMessage(self.message_text,shiftamt)  #take encrypted text, make it into a plaintextmessage object, shifting it shiftamt times
            temp = Message(test.get_message_text_encrypted())    #turns the shifted text into a message to use get valid words
            if matches < len(temp.get_valid_words()):           # if number of valid words in shifted text is greater than any previous number of matches
                matches = len(temp.get_valid_words())           # then set matches to new highest number of matches
                finalshift = test.get_shift()                   #remember the shift amount used that had most matches
                finaltext = test.get_message_text_encrypted()   #remember the text that had most matches
                msglen = len(temp.msglist)                      #remember the length of the message that had most matches
            else:
                shiftamt += 1
        if finalshift < 26:
...
if __name__ == '__main__':
    ...
    plaintextmessage = PlaintextMessage(messageclass.get_message_text(),23)
